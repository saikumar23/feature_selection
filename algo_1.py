...
def final_data_algo1(X,wn,alpha,k):
    s = singular_values(X)
    r = effective_rank(alpha,s)
    rs,svs,wns,w_mat = rs_svs_wns(X,wn,alpha,k)
    sv1,sv2 = get_sv(rs,svs,wns)
    wnt = plot_peaks(sv1,wns,rs,alpha,k)
    idxs = []
    for i in wnt:
        idxs.append(wn.index(i))
    r_spectra = get_rs_matrix(X,idxs)
    S = singular_values(r_spectra)
    r_t = effective_rank(alpha,S)
    flag=0
    if r == r_t:
        flag = 1
    return r_spectra,wnt,idxs
...
data,wnt,pks = final_data_algo1(X_train,wn,0.98,2)
X_test1 = get_rs_matrix(X_test,pks)
rmsep1,comps,std1,new_rmsep1 = pls_model(X_train,X_test,y_train,y_test)
rmsep2,comps,std1,new_rmsep2 = pls_model(data,X_test1,y_train,y_test)
print("All wavelengths: {} , RMSEP: {}".format(len(X[0]),new_rmsep1))
print("Algorithm 1: {} , RMSEP: {}".format(len(data[0]),new_rmsep2))